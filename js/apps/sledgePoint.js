...
<div class="sp-main">
  ...
  <section class="sp-work">
    ...
    function htmlExport(state) {
        ...
        const slideMarkup=state.doc.slides.map((s,i)=>`<section class="slide" style="display:${i?'none':'block'};background:${s.background||'#fff'}"><div class="slide-scale" data-cw="${cw}" data-ch="${ch}">${s.elements.map(exportElement).join('')}</div></section>`).join('');
        return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(state.doc.name)}</title><style>html,body{margin:0;height:100%;background:#000;font-family:Segoe UI,Arial}.slide{width:100vw;height:100vh;max-width:177.777vh;max-height:56.25vw;position:absolute;inset:0;margin:auto;overflow:hidden}.slide-scale{position:absolute;inset:0;transform-origin:top left;width:100%;height:100%}.e{position:absolute;box-sizing:border-box}.text{white-space:pre-wrap}.shape.ellipse{border-radius:50%}table{border-collapse:collapse;width:100%;height:100%}td{border:1px solid #777;padding:4px}img{width:100%;height:100%;object-fit:fill}</style></head><body>${slideMarkup}<script>function scaleSlides(){document.querySelectorAll('.slide-scale').forEach(el=>{const cw=+el.dataset.cw,ch=+el.dataset.ch,sx=el.parentElement.clientWidth/cw,sy=el.parentElement.clientHeight/ch;el.style.transform='scale('+Math.min(sx,sy)+')'})}scaleSlides();addEventListener('resize',scaleSlides);let i=0,s=[...document.querySelectorAll('.slide')];function go(n){s[i].style.display='none';i=(n+s.length)%s.length;s[i].style.display='block'}addEventListener('keydown',e=>{if(['ArrowRight','PageDown',' '].includes(e.key))go(i+1);if(['ArrowLeft','PageUp'].includes(e.key))go(i-1);if(e.key==='Escape')document.exitFullscreen?.()});document.body.onclick=()=>go(i+1)</script></body></html>`;
    }
